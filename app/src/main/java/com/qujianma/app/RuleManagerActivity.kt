...
class RuleManagerActivity : AppCompatActivity() {
    ...
    private fun updateDialogParsingResult(dialogView: View, testSmsContent: EditText, parsingResult: TextView) {
        val smsContent = testSmsContent.text.toString()
        if (smsContent.isNotEmpty()) {
            // 创建临时规则对象
            val tempRule = Rule(
                tagPrefix = dialogView.findViewById<EditText>(R.id.tag_prefix).text.toString(),
                tagSuffix = dialogView.findViewById<EditText>(R.id.tag_suffix).text.toString(),
                codePrefix = dialogView.findViewById<EditText>(R.id.code_prefix).text.toString(),
                codeSuffix = dialogView.findViewById<EditText>(R.id.code_suffix).text.toString(),
                addressPrefix = dialogView.findViewById<EditText>(R.id.address_prefix).text.toString(),
                addressSuffix = dialogView.findViewById<EditText>(R.id.address_suffix).text.toString(),
                enabled = true
            )
